...
def get_referral_date(test, rate_parameter, rate_adjustment, weights_parameter, base_date, priority):
  # Returns a random process date for a referral 
  #
  # test:  if >0 then a date is calculated, otherwise none 
  # rate_parameter:  the name of a dictionary entry in data_parameters that contains the rate of having a process date 
  # rate_adjustment:  the amount to subtract from the rate of having a process date 
  # weights_parameter:  the name of a dictionary entry in data_parameters that contains the six weightings for the number of days past the base date 
  # base_date:  the date to add the days to to create the random process date 
  # priority:  the referral priority
  #
  # Returns: 
  #   return_date:  the datetime value calculated or the value datetime(1, 1, 1) for none 
  #   return_date_str:  the text rendering of the date value or an empty string for none 
  #   return_test:  returns a 1 if a date was calculated, 0 otherwise 
  ...
  if (test > 0): 
    ...
    if (idx < (data_parameters[rate_parameter] + rate_adjustment)): 

      # Small chance of this referral date being very different from the others 
      if (random.random() < 0.15):
        rate_adjustment = rate_adjustment * -1.0 * (random.random()+1.0) 
      ...
      idx = random.random() 
      if (idx < data_parameters[weights_parameter][0]+rate_adjustment): 
        days = round(random.random() * weighting_bins[priority][0]) 
      elif (idx < data_parameters[weights_parameter][1]+rate_adjustment): 
        days = round(random.random() * weighting_bins[priority][1]) + weighting_bins[priority][0]
      elif (idx < data_parameters[weights_parameter][2]+rate_adjustment): 
        days = round(random.random() * weighting_bins[priority][2]) + weighting_bins[priority][1] + weighting_bins[priority][0]
      elif (idx < data_parameters[weights_parameter][3]+rate_adjustment): 
        days = round(random.random() * weighting_bins[priority][3]) + weighting_bins[priority][2] + weighting_bins[priority][1] + weighting_bins[priority][0]
      elif (idx < data_parameters[weights_parameter][4]+rate_adjustment): 
        days = round(random.random() * weighting_bins[priority][4]) + weighting_bins[priority][3] + weighting_bins[priority][2] + weighting_bins[priority][1] + weighting_bins[priority][0] 
      elif (idx < data_parameters[weights_parameter][5]+rate_adjustment): 
        days = round(random.random() * weighting_bins[priority][5]) + weighting_bins[priority][4] + weighting_bins[priority][3] + weighting_bins[priority][2] + weighting_bins[priority][1] + weighting_bins[priority][0]
      else: 
        days = round(random.random() * weighting_bins[priority][6]) + weighting_bins[priority][5] + weighting_bins[priority][4] + weighting_bins[priority][3] + weighting_bins[priority][2] + weighting_bins[priority][1] + weighting_bins[priority][0]
...
record_str = 'Referral ID,Source Location,Provider Referred To,Location Referred To,Referral Priority,Referral Status,Date Referral Written,Date Referral Sent,Date Accepted,Date Referral Completed,Date Referral Scheduled,Date Referral Seen,Patient ID,Clinic,Date Similar Appt Scheduled,Date Patient Checked In,Date Held,Date Pending Reschedule,Date Last Referral Update,Last Referral Update By,Assigned Personnel,Organization Referred To,Reason for Hold,Referral Sub-Status\n' 
f.write(record_str)
...
while referral_date <= data_parameters['last_referral_date']: 
  ...
  num_referrals = round((random.random() * (data_parameters['maximum_referrals_daily'] - data_parameters['minimum_referrals_daily'])) + data_parameters['minimum_referrals_daily']) 
  ...
  while referral_num < num_referrals: 
    ...
    if (idx < data_parameters['accepted_rate']): 
      days_to_accept = round(random.random() * (data_parameters['days_to_accept'][1] - data_parameters['days_to_accept'][0])) + data_parameters['days_to_accept'][0]  
      accept_date = referral_date + relativedelta(days=days_to_accept) 
    ...
    schedule_date, schedule_date_str, scheduled = get_referral_date(accepted, 'scheduled_rate', rate_adjustment, 'referral_scheduled_weights', accept_date, referral_priority_val)

    # Randomly see referrals if they are scheduled and calculate the days to seen
    seen_date, seen_date_str, seen = get_referral_date(scheduled, 'seen_rate', rate_adjustment, 'referral_seen_weights', schedule_date, referral_priority_val)

    # Randomly select referrals to have similar appointments that aren't updated in the referral status to reflect real life data quality issue  
    similar_scheduled_date, similar_scheduled_date_str, similar_scheduled = get_referral_date(1, 'similar_scheduled_rate', rate_adjustment, 'similar_scheduled_weights', referral_written_date, referral_priority_val)

    # Randomly select referrals to have similar appointments checked in that aren't updated in the referral status to reflect real life data quality issue  
    similar_seen_date, similar_seen_date_str, similar_seen = get_referral_date(similar_scheduled, 'similar_seen_rate', rate_adjustment, 'similar_seen_weights', similar_scheduled_date, referral_priority_val)

    # Randomly select referrals to be completed, either after seen or before being seen
    if ((seen == 0) and (similar_seen == 0)):
      completed_date, completed_date_str, completed = get_referral_date(1, 'not_seen_completed_rate', rate_adjustment, 'not_seen_completed_weights', referral_written_date, referral_priority_val)
    else: 
      either_date = max(similar_seen_date, seen_date) 
      completed_date, completed_date_str, completed = get_referral_date(1, 'seen_completed_rate', rate_adjustment, 'seen_completed_weights', either_date, referral_priority_val)

    # Choose a random last update date after the latest process date 
    latest_date = max(completed_date, similar_seen_date, similar_scheduled_date, seen_date, schedule_date, accept_date, referral_written_date) 
    days_to_update = round(random.random() * (data_parameters['days_to_update'][1] - data_parameters['days_to_update'][0])) + data_parameters['days_to_update'][0]  
    update_date = latest_date + relativedelta(days=days_to_update) 
    ...
    if(referral_date > holds_start_date): 
      holds_ok = 1 
    on_hold_date, on_hold_date_str, on_hold = get_referral_date(still_pending * holds_ok, 'on_hold_rate', 0, 'on_hold_weights', referral_date, referral_priority_val)
    if(on_hold == 1): 
    ...
    pending_reschedule_date, pending_reschedule_date_str, pending_reschedule = get_referral_date(still_pending * holds_ok, 'pending_reschedule_rate', 0, 'pending_reschedule_weights', referral_date, referral_priority_val)
    ...
    record_str = f'{referral_id},"{source_location_val}","{provider_referred_to_val}","{location_referred_to_val}","{referral_priority_val}","{referral_status}",{referral_written_date_str},{referral_date_str},{accept_date_str},{completed_date_str},{schedule_date_str},{seen_date_str},{patient_id},"{clinic_val}",{similar_scheduled_date_str},{similar_seen_date_str},{on_hold_date_str},{pending_reschedule_date_str},{update_date_str},"{last_referral_update_by_val}","{assigned_personnel_val}","{organization_referred_to_val}","{reason_for_hold_val}","{referral_substatus_val}"\n' 
    f.write(record_str) 
